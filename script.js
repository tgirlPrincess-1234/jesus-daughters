...
        const response = await fetch('https://bible-api.com/?random=verse&t=' + newDate(). getTime()); 
        const data = await response.json();
        
        let finalVerseText = "";
        let finalVerseRef = "";

        // FORMAT 1: Check if the API returned an array of verses
        if (data && data.verses && data.verses.length > 0) {
            finalVerseText = data.verses[0].text;
            finalVerseRef = data.verses[0].reference;
        } 
        // FORMAT 2: Check if it returns a direct text object
...
        // Clean up any weird spacing formats from the API data stream
